...
// Founder background, as supplied by the founder.
const CREDS = [
  [GraduationCap, 'Network engineering undergrad, MIT Melbourne'],
  [ShieldCheck, 'Cyber security major'],
  [MapPin, 'Sri Lankan, studying in Melbourne'],
];
    ...
    <section className="section pf-about" id="about">
      <div className="wrap pf-about-in">
        <Reveal className="pf-note">
          ...
          <ul className="pf-creds" aria-label="Background">
            {CREDS.map(([Icon, label]) => (
              <li key={label}>
                <Icon className="lucide svg" aria-hidden="true" />
                {label}
              </li>
            ))}
          </ul>
